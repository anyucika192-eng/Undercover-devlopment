--==================================================
-- Project Undercover Build20250205 - ANIMATED PROFESSIONAL UI (FIXED)
--==================================================
...
local Configuration = {
    ...
    ESPBox = false,
    ESPBoxFilled = false,
    CorneredBox = false, -- NEW: Cornered box option
    CornerLength = 6, -- NEW: Length of corner lines
    NameESP = false,
    HealthESP = false,
    TracerESP = false,
    SkeletonESP = false,
...
gui.Enabled = false -- Hidden initially for loading screen

-- ================= LOADING SCREEN WITH FAKE ANTI-CHEAT =================
...
local function ShowLoadingScreen()
    local loadTime = math.random(30, 45) / 10 -- 3.0 to 4.5 seconds
    local steps = 100
    local currentStep = 0
    local stageIndex = 1
    
    while currentStep < steps do
        -- Random jump between 1-4%
...
accentDot.Position = UDim2.new(0.5, 52, 0.5, -3)
...
-- ================= PLAYER PROFILE SECTION (Bottom of sidebar) =================
...
-- Update avatar when character loads
...
-- ================= ATTACHED ESP PREVIEW (Same theme as menu) =================
...
-- Preview title bar (matching menu theme)
...
local pCorneredBoxLines = {} -- NEW: Lines for cornered box
for i = 1, 8 do
...
-- Helper function to draw cornered box
local function updateCorneredBox(boxPos, boxSize, color, thickness, transparency, cornerLength)
    local tl = boxPos -- top-left
    local tr = Vector2.new(boxPos.X + boxSize.X, boxPos.Y) -- top-right
    local bl = Vector2.new(boxPos.X, boxPos.Y + boxSize.Y) -- bottom-left
    local br = Vector2.new(boxPos.X + boxSize.X, boxPos.Y + boxSize.Y) -- bottom-right
...
-- Function to update preview position based on menu
...
-- Connect to menu position changes
...
local function addSlider(text, min, max, default, callback, suffix)
    ...
    labelRow.Position = UDim2.new(0, 12, 0, 6)
...
local function switchTab(tabName)
    ...
    -- Show/hide preview based on tab
    ...
    elseif tabName == "Rage" then
        -- Empty Rage tab - coming soon
        local placeholder = Instance.new("Frame", contentScroll)
        placeholder.Size = UDim2.new(1, 0, 0, 120)
    ...
    elseif tabName == "Visuals" then
        ...
        addElement(addToggle("Box ESP", Configuration.ESPBox, function(v) 
            ...
            if v then Configuration.CorneredBox = false end -- Disable cornered box when box is enabled
        ...
        addElement(addToggle("Cornered Box", Configuration.CorneredBox, function(v) 
            ...
            if v then Configuration.ESPBox = false end -- Disable box when cornered box is enabled
            ShowingESP = Configuration.ESPBox or Configuration.CorneredBox or Configuration.NameESP or Configuration.HealthESP or Configuration.TracerESP or Configuration.SkeletonESP or Configuration.HighlightESP or Configuration.HeadCircle
        end))
...
local Highlights = {}
local HeadCircles = {}
local CorneredBoxLines = {} -- NEW: Store cornered box line objects
...
local function newESP(plr)
    ...
    CorneredBoxLines[plr] = {} -- Initialize cornered box lines
...
RunService.RenderStepped:Connect(function(dt)
    ...
    -- ESP Preview (always visible, same theme)
    if previewWindow.Visible then
        ...
        -- Name ESP - FIXED to be more visible with outline
    ...
    for plr, esp in pairs(ESP) do
        ...
        if char and hum and hrp and hum.Health > 0 and plr ~= LocalPlayer then
            ...
            -- Only render ESP if on screen and ESP is enabled
            if onScreen and ShowingESP then
                ...
                if headOn and footOn then
                    ...
                    -- Cornered Box ESP - NEW
                    if Configuration.CorneredBox then
                        -- Clear old cornered box lines
                    ...
                    -- Name ESP - FIXED to be more visible
